...
, LEAST(0.0 + sum(fy.amount / tky.factor) / 500e3, 1.0) as lerp_vol
, log( avg(fx.amount / fy.amount * tky.factor / tkx.factor) ) * 0.25 + 0.5 as lerp_log
from transactions txs
join ft_events fy
    on (txs.tx_id = fy.tx_id and sender_address in (fy.sender, fy.recipient)
        and fy.asset_identifier = 'SP125J1ADVYWGWB9NQRCVGKYAG73R17ZNMV17XEJ7.slime-token::SLIME')
join stx_events fx
