...
, round(yr.value_btc, 4)::text as "fees(BTC), year"
, round(mo.value_btc, 4)::text as "fees(BTC), eoy"
, round((mo.value_btc / yr.value_btc - 1) * 100, 1)::text "Δfees(BTC), eoy/yr (%)"
, round((yr.value_btc / lag(yr.value_btc) over (order by yr.ts) - 1) * 100, 1)::text as "Δfees(BTC), yr/yr (%)"
from annual yr
