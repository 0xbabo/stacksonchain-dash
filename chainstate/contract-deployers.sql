with known_accounts (address, description) as (VALUES
    ...
    ('SP3XYJ8XFZYF7MD86QQ5EF8HBVHHZFFQ9HM6SPJNQ', 'BNS Market'),
    ...
    ('SPV0WDJQHWJVR2XD59H5NGX9AMNNTP4Q92XV77JD', 'Junk/Spam')
)

select split_part(contract_id,'.',1) as deployer
...
left join (
    ...
    from stxop.bns_address join transactions using (tx_hash)
    order by address, block_height desc
) bns on (address = split_part(contract_id,'.',1))
