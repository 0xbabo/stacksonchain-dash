...
, events as (
    ...
    where p.contract_id in (ft_mia_v2, ft_nyc_v2)
...
CASE address
    WHEN miawallet THEN 'miamiwallet.btc'
    WHEN nycwallet THEN 'nycwallet.btc'
    ELSE address
    END as account
-- , token
, sum(amount*price) as "Value (STX)"
from base
join token_prices p using (token)
cross join const
-- left join stxop.bns_address using (address)
-- where address in (miawallet, nycwallet)
-- where token = ft_wstx
where address in (
    miawallet, nycwallet, core_mia_v2, core_nyc_v2
)
-- and token = ft_wstx
group by 1
order by 2 desc
